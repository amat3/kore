      ...
      /* @__PURE__ */ jsxs7(ImageWrapper, { className: "kore-card-image", children: [
        ...
        (favorited || onFavorite) && /* @__PURE__ */ jsx13(
          FavoriteButton,
          {
            ...
            $active: favorited,
            children: /* @__PURE__ */ jsx13(Icon_default, { name: "Heart", size: "sm", color: "inherit" })
          }
        )
      ] }),
...
var CompletedBadge = styled10.div`
  ...
  justify-content:  center;
`;
var FavoriteButton = styled10.button`
  position:         absolute;
  top:              var(--spacing-xs);
  right:            var(--spacing-xs);
  background:       ${({ $active }) => $active ? "var(--background-accent-solid)" : "rgba(0,0,0,0.45)"};
  border:           none;
  ...
  @media (hover: hover) {
    &:hover {
      background: ${({ $active }) => $active ? "color-mix(in srgb, var(--background-accent-solid), black 15%)" : "rgba(0,0,0,0.65)"};
      transform: scale(1.1);
    }
  }
`;
